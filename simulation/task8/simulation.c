...
const double MIN_NOISE_POWER = 0.000001;
...
int main(void) {
    ...
    FILE *snrOutput = fopen("data/task8/SNR_output.dat", "w");
    ...
    while(NOISE_POWER > MIN_NOISE_POWER){
        ...
        while(currentTime < MAX_TIME_LONG && isRunning){
            ...
            printf(BLU "\rRun: %d - Simulation Progress: %0.2lf%%" RESET,runCount, percentComplete);
            fflush(stdout);
        ...
        fprintf(snrOutput, "%0.4lf\n",SAMPLES_PER_SYMBOL/(2*NOISE_POWER));
